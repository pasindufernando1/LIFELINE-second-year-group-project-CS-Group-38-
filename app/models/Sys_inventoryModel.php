...
public function getAllInventory($id)
{
    ...
    public function getfilterdoninv($blood_bank_id,$start,$end)
    {
        $params = array(':blood_bank_id',':start',':end');
        $inputs = array($blood_bank_id,$start,$end);
        $invdon = $this->db->select("*","inventory_donation","
        INNER JOIN organization_donations_bloodbank ON inventory_donation.DonationID = inventory_donation.DonationID 
        INNER JOIN organization_society ON organization_society.UserID = organization_donations_bloodbank.OrganizationUserID
        INNER JOIN donation ON donation.DonationID = inventory_donation.DonationID
        WHERE organization_donations_bloodbank.BloodBankID =:blood_bank_id
        AND donation.Initialized_date >= :start
        AND donation.Initialized_date <= :end
        AND inventory_donation.Accepted_date IS NULL
        ",$params,$inputs);
        return $invdon;
    }

    public function getfilterdoninvf($blood_bank_id,$start,$end)
    {
        $params = array(':blood_bank_id',':start',':end');
        $inputs = array($blood_bank_id,$start,$end);
        $invdon = $this->db->select("*","inventory_donation","
        ...
        AND donation.Initialized_date >= :start
        AND donation.Initialized_date <= :end
        AND inventory_donation.Accepted_date IS NOT NULL
        ",$params,$inputs);
        return $invdon;
    }
