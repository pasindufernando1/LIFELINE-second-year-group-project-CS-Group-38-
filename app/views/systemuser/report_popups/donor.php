...
<div id="r05" class="modal">
  <span onclick="document.getElementById('r05').style.display='none'" class="close" title="Close Modal">×</span>
  <form class="modal-content" id="del" action="/sys_reports?page=1&report=r5generated" method="POST">
    ...
    <div>
      ...
      <div class="clearfix">
        <button type="button" onclick="document.getElementById('r05').style.display='none'" class="cancelbtn">Cancel</button>
        <button name="r5" type="submit" onclick="document.getElementById('r05').style.display='none'" class="deletebtn">Generate</button>
      </div>
    </div>
  </form>
</div>
<script>
// Get the modal
var modal = document.getElementById('r05');
...
nic?.addEventListener("input", function () {
    ...
    if ((!reg1.test(nic.value)) && (!reg2.test(nic.value))) {
        ...
        nicLabel.innerHTML = "NIC of Donor";
        nicLabel.style.color = "#640e0b";
        nic_flag = true;
